...
%   Copyright 2014 The MathWorks, Inc.
   ...
   H = initialize_graphics(P,gui,erasemode);
   ...
   if erasemode
      if trace
         for i = 1:n
            ...
            set(H.bodies(i),'markersize',ms,'erasemode','none')
         end
         set(H.clock,'erasemode','xor')
      else
         for i = 1:n
            ms = get(H.bodies(i),'userdata');
            set(H.bodies(i),'markersize',ms,'erasemode','normal')
         end
      ...
      if trace
         for i = 1:n
            set(H.bodies(i),'vis','off')
            ms = 6;
            if n == 9 && i == 1, ms = 24; end
            H.bodies(i) = animatedline(NaN,NaN,NaN, ...
               'color',get(H.bodies(i),'color'), ...
               'marker','.','markersize',ms, ...
               'userdata',get(H.bodies(i),'userdata'));
         end
      else
         for i = 1:n
            set(H.bodies(i),'vis','off')
            [xi,yi,zi] = getpoints(H.bodies(i));
            ms = get(H.bodies(i),'userdata');
            H.bodies(i) = line(xi,yi,zi, ...
               'color',get(H.bodies(i),'color'), ...
               'marker','.','markersize',ms, ...
               'userdata',get(H.bodies(i),'userdata'));
         end
