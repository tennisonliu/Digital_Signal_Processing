...
% ------------------------------------------------
   ...
   function h = initialize_graphics
      clf
      shg
      set(gcf,'menubar','none','numbertitle','off','name','Lissajous', ...
          'color','white') 
      h = initialize_plot;
...
% ------------------------------------------------
   ...
   function h = initialize_plot
      cla
      mode = get(findobj('tag','mode'),'value');
      if isempty(mode) || mode==0
         if erasemode
            h = plot3(0,0,0,'.','erasemode','none');
         else
            h = animatedline(0,0,0,'marker','.','markersize',4, ...
               'color',[0 0 2/3]);
         end
      else
...
% ------------------------------------------------
   ...
   function plotdot(h,x,y,z)
      % Move dot on plot.
      h = get(gca,'child');
      if erasemode || isequal(get(h,'type'),'line')
